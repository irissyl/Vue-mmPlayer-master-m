<template>
  <el-container>
    <el-main v-loading="loadingAll" :style="{height:BoxdefaultHeight}">
      <div id="hyAlbum" class="lead" style="font-size: 26px;">华语乐坛</div>
      <el-container>
        <div class="row" >
  ...
  methods: {
    async getDeafultData() {
      if (this.getAlbumData.length == 0) {
        ...
        await axios.get("http://localhost:8081/newmusic/newalbum/0/ZH").then((response) => {
    ...
    nextOffset() {
      ...
      if (this.AlbumOffset < 0) {
        ...
        axios.get("http://localhost:8081/newmusic/newalbum/"+this.AlbumOffset+"/ZH").then((response) => {
    ...
    prevOffset() {
      if (this.AlbumOffset <= 0) {
        ...
        console.log("点击上一页即将进入跳转行")
        this.moveScrollIntoViewhy()
        console.log("上一页" + this.AlbumOffset)
      }
    },
    moveScrollIntoViewhy() {
      // let ele = document.getElementById('hyAlbum')
      setTimeout(() => {
    ...
    toFirstData() {
      this.AlbumOffset = 0;
      axios.get("http://localhost:8081/newmusic/newalbum/0/ZH").then((response) => {
        this.getAlbumData = response.data
      })
      this.moveScrollIntoViewhy()
    }
