<template>
  <el-container>
    <el-main v-loading="loadingAll" :style="{height:BoxdefaultHeight}">
      <span id="om" class="lead" style="font-size: 26px;">欧美</span>
      <el-container>
        <div class="row">
  ...
  methods:{
    async getDeafultData(){
      if (this.getAlbumData.length == 0) {
        ...
        await axios.get("http://localhost:8081/newmusic/newalbum/0/EA").then((response) => {
    ...
    nextOffset() {
      ...
      if (this.AlbumOffset < 0) {
        ...
        axios.get("http://localhost:8081/newmusic/newalbum/"+this.AlbumOffset+"/EA").then((response) => {
        ...
        // this.moveScrollIntoViewom({behavior: 'smooth', block: 'start', inline: 'nearest'})
        this.moveScrollIntoViewom()
        console.log("下一页" + this.AlbumOffset)
      }
    },
    prevOffset() {
    ...
    moveScrollIntoViewom() {
      console.log(1)
      // let ele = document.getElementById('om')
      setTimeout(() => {
    ...
    toFirstData(){
      this.AlbumOffset = 0;
      axios.get("http://localhost:8081/newmusic/newalbum/0/EA").then((response) => {
        this.getAlbumData = response.data
      })
    }
  },
