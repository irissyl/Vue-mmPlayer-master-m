<template>
  <el-container>
    <el-main v-loading="loadingAll" :style="{height:BoxdefaultHeight}">
      <span id="jp" class="lead" style="font-size: 26px;">日本</span>
      <el-container>
        <div class="row">
  ...
  methods:{
    async getDeafultData(){
      if (this.getAlbumDatas.length == 0) {
        ...
        await axios.get("http://localhost:8081/newmusic/newalbum/0/JP").then((response) => {
    ...
    nextOffset() {
      ...
      if (this.AlbumOffset < 0) {
        ...
        axios.get("http://localhost:8081/newmusic/newalbum/"+this.AlbumOffset+"/JP").then((response) => {
        ...
        // this.moveScrollIntoViewjp({behavior: 'smooth', block: 'start', inline: 'nearest'})
        this.moveScrollIntoViewjp()
        console.log("下一页" + this.AlbumOffset)
      }
    },
    prevOffset() {
    ...
    moveScrollIntoViewjp() {
      // let ele = document.getElementById('jp')
      setTimeout(() => {
    ...
    toFirstData(){
      this.AlbumOffset = 0;
      axios.get("http://localhost:8081/newmusic/newalbum/0/JP").then((response) => {
