<template>
  <el-container>
    <el-main v-loading="loadingAll" :style="{height:BoxdefaultHeight}">
      <span id="kor" class="lead" style="font-size: 26px;">韩国</span>
      <el-container>
        <div class="row">
          <div class="Album-style" v-for="(item,index) in getAlbumDatas" :key="index" v-loading="loadingAll" :style="{height:BoxdefaultHeight}">
...
export default {
  ...
  methods:{
    async getDeafultData(){
      if (this.getAlbumDatas.length == 0) {
        this.loadingAll = true
        this.BoxdefaultHeight = '700px'
        await axios.get("http://localhost:8081/newmusic/newalbum/0/KR").then((response) => {
          this.getAlbumDatas = response.data
        })
    ...
    nextOffset() {
      ...
      if (this.AlbumOffset < 0) {
        ...
        axios.get("http://localhost:8081/newmusic/newalbum/"+this.AlbumOffset+"/KR").then((response) => {
          this.tempAlbum = response.data
          if (this.tempAlbum.length != 0) {
            this.getAlbumDatas = this.tempAlbum
          } else {
            layer.msg("已经到最后一页了~"),
                this.AlbumOffset = this.AlbumOffset - 20;
          }
        })
        // this.moveScrollIntoViewkor({behavior: 'smooth', block: 'start', inline: 'nearest'})
        this.moveScrollIntoViewkor()
        console.log("下一页" + this.AlbumOffset)
      }
    },
    prevOffset() {
    ...
    moveScrollIntoViewkor() {
      // console.log("kor")
      // let ele = document.getElementById('kor')
      setTimeout(() => {
    ...
    toFirstData(){
      this.AlbumOffset = 0;
      axios.get("http://localhost:8081/newmusic/newalbum/0/KR").then((response) => {
